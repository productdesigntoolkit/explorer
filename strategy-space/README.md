# 🎯 Strategy Space Intro

## Strategische Grundlagen und Positionierung

Willkommen im **Strategy Space**! Hier findest du alle Methoden dieser Phase deiner Produktentwicklung.

## Alle Methoden in diesem Space

* [BCG Matrix](BCG_Matrix.md)
* [Blue Ocean 4 Actions Framework](Blue_Ocean_4_Actions_Framework.md)
* [Business Model Canvas](Business_Model_Canvas.md)
* [Ecosystem Canvas](Ecosystem_Canvas.md)
* [Impact Mapping (Strategy)](Impact_Mapping_Strategy_.md)
* [Innovationsmatrix](Innovationsmatrix.md)
* [Market Sizing TAM SAM SOM](Market_Sizing_TAM_SAM_SOM_.md)
* [Marktstrategie](Marktstrategie.md)
* [North Star Metrics](North_Star_Metrics.md)
* [OKR Framework](OKR_Framework.md)
* [PESTEL Analyse](PESTEL_Analyse.md)
* [Porters Five Forces](Porters_Five_Forces.md)
* [Pricing Strategy Canvas](Pricing_Strategy_Canvas.md)
* [Product Lifecycle](Product_Lifecycle.md)
* [Produktstrategie](Produktstrategie.md)
* [Seven Powers](seven-powers.md)
* [STP Model](STP_Model.md)
* [SWOT Analyse](SWOT_Analyse.md)

## Wann diese Methoden nutzen?

Die Methoden im Strategy Space helfen dir bei:

* Systematischer Arbeit im Strategy Space
* Strukturiertem Vorgehen
* Praxiserprobten Frameworks

## Tipps für diesen Space

1. **Kombiniere Methoden** - Keine Methode funktioniert isoliert
2. **Adaptiere** - Passe die Methoden an deinen Kontext an
3. **Iteriere** - Spring zurück wenn neue Erkenntnisse auftauchen

***

**Zurück zur** [Hauptübersicht](../)
